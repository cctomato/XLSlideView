...
@interface XLLineScrollTabbarView ()
...
@implementation XLLineScrollTabbarView
...
- (void)clickTitle:(UITapGestureRecognizer *)tap
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(XLLineScrollTabbar:selectAt:)]) {
        [self.delegate XLLineScrollTabbar:self selectAt:newSelectedIndex];
    }
}
    ...
    if (index < self.itemArray.count && index >= 0) {
        if (self.fromIndex != -1) {
            ...
            [UIView animateWithDuration:self.animationTime animations:^{
                ...
                self.selectedBackgroundView.frame = CGRectMake(newTitleLabel.frame.origin.x, self.scrollView.frame.size.height - 1, newTitleLabel.frame.size.width, 1);
...
- (void)layoutSubviews
{
    ...
    for (NSInteger index = 0; index < self.itemArray.count; index++) {
        ...
        if (index == self.fromIndex) {
            self.selectedBackgroundView.frame = CGRectMake(titleLabel.frame.origin.x, self.scrollView.frame.size.height - 1, titleLabel.frame.size.width, 1);
        }
...
- (UIView *)selectedBackgroundView
{
    if (_selectedBackgroundView == nil) {
        ...
        _selectedBackgroundView.backgroundColor = self.selectedColor;
    }
